...
FastAPI server for the AI Job Application Agent.
  ...
  /api/*   JSON endpoints, plus a live event stream for agent runs
...
class RunRequest(BaseModel):
    ...
    jd_source: str = Field(default="pasted text", max_length=500)
    recipient: str = Field(default="", max_length=320)
    allow_send: bool = False
    user_id: str = Field(default="", max_length=200, pattern=r"^[\w.@+-]*$")
    slack_channel: str = Field(default="", max_length=80, pattern=r"^#?[\w.-]*$")
...
async def run(req: RunRequest):
    ...
    def on_event(event: dict) -> None:  # called from the worker thread
