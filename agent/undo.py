...
independently, so one failure doesn't stop the others. A simulated run is undone
in simulated mode, so it never touches real accounts.
"""
...
from agent.utils import SIMULATED, classify_google_error
...
def undo_run(run_id: str) -> dict | None:
    ...
    token = SIMULATED.set(bool(entry.get("simulated")))
    try:
