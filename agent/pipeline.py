"""
Orchestrates the 3-agent pipeline: Researcher (extract) -> Tailor -> Executor
(independent fit review + dispatch to external apps), and writes an eval log
entry for every run so results can be reviewed after a test batch.

5 external apps are integrated in total:
...
def run_pipeline(resume_text: str, jd_text: str, company: str, role: str,
    ...
    """
    Runs the 3-agent pipeline (Researcher -> Tailor -> Executor).
    progress_cb(step_name, status) is called at the start/end of each step
    so a UI can render live progress.
    ...
    # Agent 1: Researcher — extract what the JD actually wants
    tick("researcher", "running")
    requirements, extract_live = extract_requirements(jd_text)
    tick("researcher", "done")

    # Agent 2: Tailor — rewrite resume + cover note against those requirements
    tick("tailor", "running")
    tailoring, tailor_live = tailor_application(resume_text, requirements)
    tick("tailor", "done")

    # Agent 3: Executor — rule gate + LLM second opinion, then dispatch
    tick("executor", "running")

    guardrail = score_overlap(resume_text, requirements)
    verdict, executor_live = execute_review(resume_text, requirements, guardrail)

    # The rule gate stays authoritative; the LLM verdict is advisory.
    # Gmail/Sheets/Calendar/Drive skipped when flagged for review; Slack
    # always fires so a human hears about the outcome either way. Gmail
    # auto-sends only when the stricter AUTO_SEND_THRESHOLD gate (on top of
    # the normal guardrail already passing) is cleared — otherwise it's a
    # draft for a human to review and send themselves.
    actions = {}
    ...
    tick("executor", "done" if not guardrail["needs_review"] else "skipped")
    ...
    result = {
        ...
        "actions": actions,
        "used_live_llm": extract_live and tailor_live and executor_live,
        "passed": passed,
    }
